<template>
  <v-container fluid class="registro-container">
    <v-row justify="center" align="center" class="fill-height">
      <v-col cols="12" sm="8" md="4">
        <v-card class="pa-6 registro-box" elevation="10">
          <v-btn icon class="close-btn" @click="cerrar">
          ...
          <v-card-text>
            <v-form @submit.prevent="registrarUsuario">
              ...
              <v-select v-model="rol" :items="roles" item-title="nombre" item-value="id" label="Rol" required
                variant="outlined" class="mb-3" />


              <v-text-field v-model="correo" label="Correo" type="email" required variant="outlined"
                class="mb-3"></v-text-field>
              <v-text-field v-model="password" label="Contraseña" :type="showPassword ? 'text' : 'password'"
                variant="outlined" required class="mb-3" :append-inner-icon="showPassword ? 'mdi-eye' : 'mdi-eye-off'"
                @click:append-inner="togglePassword">
              </v-text-field>
              <v-text-field v-model="confirmPassword" label="Confirmar contraseña"
                :type="showConfirmPassword ? 'text' : 'password'" variant="outlined" required class="mb-3"
                :append-inner-icon="showConfirmPassword ? 'mdi-eye' : 'mdi-eye-off'"
                @click:append-inner="toggleConfirmPassword">
              </v-text-field>
              <v-btn type="submit" block class="register-btn">REGISTRAR USUARIO</v-btn>
            </v-form>
          </v-card-text>
        </v-card>
      </v-col>
...
<style scoped>
/* ✅ SOLUCIÓN GARANTIZADA: el fondo cubrirá TODA la pantalla sin franjas blancas */
html,
body {
  height: 100%;
  margin: 0;
...
.registro-container {
  ...
  background: url("@/assets/imgRegistro.png") no-repeat center center fixed;
  background-size: cover;
}

/* Ajustes a la tarjeta del formulario */
.registro-box {
  border-radius: 20px;
  text-align: center;
}
...
/* ✅ Botón de registro con degradado morado */
.register-btn {
  font-size: 1rem;
  font-weight: bold;
  color: white;
